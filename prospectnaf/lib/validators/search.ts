...
export const SearchSchema = z.object({
  ...
  dateFrom: z.string().regex(/^\d{4}(-\d{2}-\d{2})?$/, 'Format attendu : AAAA ou AAAA-MM-JJ').optional().or(z.literal('')),
  dateTo: z.string().regex(/^\d{4}(-\d{2}-\d{2})?$/, 'Format attendu : AAAA ou AAAA-MM-JJ').optional().or(z.literal('')),
